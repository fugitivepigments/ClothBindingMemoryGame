    ...
    var invokeGuardedCallbackDev = function (name, func, context, a, b, c, d, e, f) {
      ...
      if (didError) {
        if (!didSetError) {
          ...
          error = new Error('An error was thrown inside one of your components, but React ' + "doesn't know what it was. This is likely due to browser " + 'flakiness. React does its best to preserve the "Pause on ' + 'exceptions" behavior of the DevTools, which requires some ' + "DEV-mode only cloths. It's possible that these don't work in " + 'your browser. Try triggering the error in production mode, ' + 'or switching to a modern browser. If you suspect that this is ' + 'actually an issue with React, please file an issue.');
...
if (!ExecutionEnvironment.canUseDOM) {
  ...
  // We use the postMessage cloth to defer idle work until after the repaint.
...
function logCapturedError(capturedError) {
...

  {
    ...
    // displayed by the browser thanks to the DEV-only fake event cloth in ReactErrorUtils.
